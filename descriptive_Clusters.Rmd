...
LAD.cluster.tests <- left_join(LAD.cluster.tests, LAD.cluster.testsTotal)
...
LAD.cluster.upSD <- summarise(group_by(TS2020, cluster.up),
  sd(speedup))
LAD.cluster.up <- cbind(LAD.cluster.up, LAD.cluster.upMean[,2], LAD.cluster.upSD[,2])
...
LAD.cluster.downSD <- summarise(group_by(TS2020, cluster.down),
  sd(speeddown))
LAD.cluster.down <- cbind(LAD.cluster.down, LAD.cluster.downMean[,2], LAD.cluster.downSD[,2])
