...
```{r}
data <- list(lad, ladDist, job.dens, pop, pop.dens, labour, earnings, busi) # ns 
...
data$cluster.up <- relevel(data$cluster.up, "5") # base category: cluster 5

aux <- multinom(cluster.up ~ 
...
stargazer(aux, 
          ...
          covariate.labels=c(#"pop. 16-64, 2018",
                             ...
                             "n. of AM broadband tests per hab., 2019"),
...
data$cluster.tests <- relevel(data$cluster.tests, "5") # base category: cluster 5

aux <- multinom(cluster.tests ~ 
                  #pop16_64 + # non sign, while pop is
                  pop +
                  #pop.dens + 
                  job.dens2018 + # pop.dens is still insignificant when job.dens is excluded
...
# McFadden's R squared
aux.null <- multinom(cluster.tests ~ 1, data = data)
r2 <- 1-logLik(aux)/logLik(aux.null)
...
data$cluster.down <- relevel(data$cluster.down, "2") # base category: cluster 2

aux <- multinom(cluster.down ~ 
                  #pop16_64 + # non sign, while pop is
                  pop +
                  #pop.dens + 
                  job.dens2018 + # pop.dens is still insignificant when job.dens is excluded
