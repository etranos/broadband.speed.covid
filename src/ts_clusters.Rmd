...
help.dates <- tibble(help.date.time = c(make_datetime(year = 2020, month = 2, day = 3),
                make_datetime(year = 2020, month = 2, day = 4),
                make_datetime(year = 2020, month = 2, day = 5),
                make_datetime(year = 2020, month = 2, day = 6),
                make_datetime(year = 2020, month = 2, day = 7)),
...
ts.wide <- ts.wide %>%
  select(help.date.time, LAD19NM, mean.up) #%>%
...
pc_k <- tsclust(ts.wide, k = 10, seed = 94L, 
                type="partitional", 
                distance = "dtw_basic", centroid = "pam", trace = T, 
                args = tsclust_args(dist = list(window.size = 20L)))
plot(pc_k)
...
ts.wide <- ts.wide %>%
  select(help.date.time, LAD19NM, mean.down) #%>%
  #filter(date > "2020-2-15")
...
ts.wide <- ts.wide %>%
  select(help.date.time, LAD19NM, n.tests) #%>%
  #filter(date > "2020-2-15")
...
pc_k <- tsclust(ts.wide, k = 5, seed = 94L, 
                type="partitional", 
                distance = "dtw_basic", centroid = "pam", trace = T, 
                args = tsclust_args(dist = list(window.size = 20L)))
plot(pc_k)
...
# create a 'fake' week for a ts id
help.dates <- tibble(help.date.time = c(make_datetime(year = 2019, month = 2, day = 4),
                make_datetime(year = 2019, month = 2, day = 5),
                make_datetime(year = 2019, month = 2, day = 6),
                make_datetime(year = 2019, month = 2, day = 7),
                make_datetime(year = 2019, month = 2, day = 8)),
                weekday = c("Mon", "Tue", "Wed", "Thu", "Fri"))
...
# save clusters file
out.clusters <- paste(path, "./data/temp/clusters.csv", sep = "")
...
# 
# var <- la@data[ ,'cluster.up.diff']
# plot(la, col = var, main = "Upload difference clusters",
#      border = rgb(0.8, 0.8, 0.8, 0))
# 
# 
...
leaflet(la) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("cluster.up","cluster.down", "cluster.down", "cluster.up.diff", "cluster.down.diff", "cluster.tests"),
