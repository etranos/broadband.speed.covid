...
### LAD, Upload speed clusters, 2020 

```{r, echo=FALSE, warning=FALSE, message = FALSE, error=FALSE, fig.height=10, fig.asp=T}
#out.width='100%'
...
la.f <- merge(la.f, for.tip.mean.up.pm, by.x = "id", by.y = "LAD19NM", all.x=T)

#sapply(la.f, function(x) sum(is.na(x)))
#No am data points for Isles of Scilly
la.f$tip <- paste0(la.f$id, "<br>",
                   "Mean upload AM speed: ", round(la.f$mean.up.am), " Kbps", "<br>",
                   "Mean upload PM speed: ", round(la.f$mean.up.pm), " Kbps")
...
### LAD mean AM and PM upload speeds
